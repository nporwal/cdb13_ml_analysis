...
def shell_test_helper(clusters, weights, sentence):
...
def shell_test(sentence):
    clusters = pickle.load(open('brown_cluster_dict'))
    weights = pickle.load(open('models/3features10weight'))
    while(1):
        raw = raw_input('Type in a sentence that is missing a word with proper capitalization and grammar, '
                      'e.g. \'Legolas, what do your elven see?\' The program will try to predict the location of the missing word.\n')
        print '\n%s\n' % shell_test_helper(clusters, weights, raw)